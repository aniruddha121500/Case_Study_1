...
<!-- WORK ON LATER -->
<!-- ```{r, echo = TRUE} -->
<!-- cdi.df = as.data.frame(cdi)[c("State", "County", "Total_Pop")] -->
<!-- colnames(cdi.df) <- c("state", "county", "total_pop") -->
<!-- fips_needed = fips(cdi.df_reduced$state) -->
<!-- cdi.df$fips <- fips_needed -->
<!-- cdi.df_reduced = subset(cdi.df, select = c("fips", "county", "total_pop")) -->

<!-- plot_usmap(data = cdi.df_reduced, values = "total_pop", regions = "counties", color = "lightblue") +  -->
<!--   labs(title = "U.S. Counties",  -->
<!--        subtitle = "Estimated County Population in 1990") -->
